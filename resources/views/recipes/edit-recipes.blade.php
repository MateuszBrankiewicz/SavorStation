<x-app-layout>
    <x-modal name="confirm-delete-modal" :show="false" maxWidth="sm">
        <div class="p-6 text-center">
            ...
            <div class="flex justify-center gap-4">
                ...
                <button class="px-4 py-2 bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-white rounded"
                    x-on:click="$dispatch('close-modal', 'confirm-delete-modal')">
                    Anuluj
                </button>
            </div>
        </div>
    ...
    <x-modal name="confirm-update-modal" :show="false" maxWidth="lg">
        <div class="p-6 text-center">
            ...
            <div class="flex justify-center gap-4">
                ...
                <button class="px-4 py-2 bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-white rounded"
                    x-on:click="$dispatch('close-modal', 'confirm-update-modal')">
                    Anuluj
                </button>
            </div>
        </div>
    ...
    <div class="flex justify-center items-center">
        <div class="w-full max-w-3xl px-6 py-8 bg-gray-900 bg-opacity-30 rounded-md border border-black backdrop-filter backdrop-blur-sm">
            <form method="POST" id="updateForm" action="{{ route('recipes.update', $recipe->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="flex justify-center">
                        <x-primary-button type="button" class="w-full py-3 px-6" x-data x-on:click="$dispatch('open-modal','confirm-update-modal')">
                            Zaktualizuj przepis
                        </x-primary-button>
                    </div>
            </form>
            <form id="deleteForm" action="{{ route('recipes.delete', ['id' => $recipe->id]) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="button"
                    class="bg-red-600 w-full mt-3 text-white rounded-md px-3 py-2 hover:bg-red-700"
                    x-data
                    x-on:click="$dispatch('open-modal', 'confirm-delete-modal')">
                    Usuń przepis
                </button>
            </form>
        </div>
